...
export default class HelpCommand extends Command {
  ...
  async run(msg: CommandoMessage, args: HelpCommandArg): Promise<Message> {
    ...
    if (args.command && !showAll) {
      if (commands.length === 1) {
        ...
        help += `${commands[0].guildOnly ? ' (Usable only in servers)' : ''}`;
        help += `${commands[0].nsfw ? ' (NSFW)' : ''}`;
        ...
        return msg.reply(
          `Unable to identify command. Use \`/help all\` to view the list of all commands.`,
        );
      }
    } else {
      let content = `
...
__**${showAll ? 'All commands' : `Available commands in ${msg.guild}`}**__`;

      const availableCommandGroups = groups.filter((group) =>
        group.commands.some(
          (command) => !command.hidden && (showAll || command.isUsable(msg)),
        ),
      );

      availableCommandGroups.forEach((group) => {
        content += `\n\n__${group.name}__`;

        const availableCommands = group.commands.filter(
          (command) => !command.hidden && (showAll || command.isUsable(msg)),
        );
